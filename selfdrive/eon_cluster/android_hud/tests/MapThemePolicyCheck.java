...
public final class MapThemePolicyCheck {
    ...
    public static void main(String[] args) {
        ...
        check(p.current(11000) == 1, "map paused for seconds keeps the settled theme");
        check(p.current(5500 + MapThemePolicy.HOLD_MS) == 1, "held up to HOLD_MS");
        check(p.current(5501 + MapThemePolicy.HOLD_MS) == -1, "stale map falls back after HOLD_MS");
        long later = 5500 + MapThemePolicy.HOLD_MS + 5000;
        p.observe(0, later);
        check(p.current(later) == -1, "recovery cannot revive old theme");
        p.observe(0, later + 1500);
        check(p.current(later + 1500) == 0, "recovery settles");
        p.reset();
