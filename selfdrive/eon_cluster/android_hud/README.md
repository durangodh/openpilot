...
credentials, position or network are unavailable, the existing solid/vector
background remains in use.
...
EON 송신 코드와 S9 앱을 함께 업데이트해야 합니다. 새 송신부는
`hudLateralFrame: "left"`로 모델/MPC의 오른쪽 양수 Y를 왼쪽 양수 Y로
변환합니다. 레이더, 모델 비전 후보, 폰 비전, 지도 및 내비 경로는 이미
왼쪽 양수이므로 그대로 유지합니다. Z와 차량 제어 데이터는 변경하지 않습니다.

설정의 **S9 HUD SCENE FLIP** (`EonClusterHudPathFlip`)은 다음과 같습니다.

- `0`: 정상 방향 (새 설치 기본값).
- `1`: 패스, 차선, 차량, 지도 배경, 가드레일과 BSD 표시를 함께 좌우 반전.

반전은 공통 화면 투영에서 적용되므로 내부 좌표 비교와 차량 중복 제거,
가드레일 바깥쪽 방향, 지도 경로 일치 판정에는 영향을 주지 않습니다.
값은 송신부 파라미터 캐시 주기(약 1초) 이후 반영됩니다.
기존 저장값은 유지하며, 과거처럼 `0`을 강제로 `1`로 바꾸지 않습니다.
업데이트 후 정상 방향은 `0`으로 확인하세요.
`EonClusterHudMirror`는 글자까지 포함하는 전체 화면 미러 설정입니다.
