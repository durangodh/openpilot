...
public class SurfaceView extends View {
 ...
 public static class Holder { public Surface getSurface(){return new Surface();} }
...
public class Rect { public int width(){return 640;} public int height(){return 384;} }""",
  "android/graphics/Bitmap.java": """package android.graphics;
public class Bitmap {
 public enum Config { ARGB_8888 } public boolean recycled=false;
...
public class CaptureCheck {
 ...
 public static void main(String[] args){
  ...
  CarrotMapCapture.capture(a,b);check(b.sent==4 && b.last.isRecycled());
  System.out.println("PASS: no-map, ads, texture, surface, unavailable/hidden, error recovery, VGX, recycling");
 }
}""",
}
...
def main():
  ...
  subprocess.run([str(args.java_home / ("bin/javac" + suffix)), "--release", "8", "-d", str(classes),
                  *files, str(Path(__file__).with_name("CarrotMapCapture.java"))], check=True)
