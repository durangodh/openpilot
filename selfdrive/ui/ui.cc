...
static void update_model(UIState *s, const cereal::ModelDataV2::Reader &model) {
  ...
  // update path
  // C3 path width. ShowPathWidth is stored as centimeters (100 = 1.00 m half-width).
  ...
  if (++path_param_frame >= UI_FREQ) {
    ...
    const int configured_width = configured_width_value.empty()
        ? 100 : std::atoi(configured_width_value.c_str());
    path_width = std::clamp(configured_width > 0 ? configured_width / 100.0f : 1.0f,
                            0.3f, 1.5f);
  }
